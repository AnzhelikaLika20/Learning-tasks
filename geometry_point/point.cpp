...
bool geometry::Point::CrossesSegment(const geometry::Segment &segment) const {
    if (segment.GetStart().GetX() == segment.GetEnd().GetX() && segment.GetStart().GetY() == segment.GetEnd().GetY()) {
        return x_coord_ == segment.GetStart().GetX() && y_coord_ == segment.GetStart().GetY();
    }
    if (segment.GetStart().GetX() == segment.GetEnd().GetY()) {
    }
...
geometry::Vector geometry::Point::operator-(geometry::Point other) const {
    return geometry::Vector(other.x_coord_ - x_coord_, other.GetY() - y_coord_);
}
