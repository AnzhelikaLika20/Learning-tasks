...
double geometry::Segment::Distance(geometry::Point point) const {
    geometry::Vector segment = GetEnd() - GetStart();
    geometry::Vector vect1 = point - GetStart();
    geometry::Vector vect2 = point - GetEnd();
    if (ScalarMult(segment, vect1) * ScalarMult(vect2, -segment) <= 0) {
        return std::min(Length(vect1), Length(vect2));
    } else {
        geometry::Line line = geometry::Line(start_, end_);
